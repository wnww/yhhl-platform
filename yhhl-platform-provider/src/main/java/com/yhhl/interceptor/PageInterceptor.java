...
public class PageInterceptor implements Interceptor {
	...
	private String getOraclePageSql(SearchPageUtil page, StringBuffer sqlBuffer) {
		...
		//sqlBuffer.insert(0,"select t2.* from (select t1.*, rownum rn from (").append(") t1 ) t2 where t2.rn between ").append(offset).append(" and ").append(page.getPageSize());
		if(page.getOrderBys()!=null && page.getOrderBys().length>0){
			sqlBuffer.append(" order by ");
			String[] orderBys = page.getOrderBys();
			for(int i=0; i<orderBys.length; i++){
				sqlBuffer.append(orderBys[i]).append(",");
			}
		}
	...
	private void setTotalRecord(SearchPageUtil page, MappedStatement mappedStatement, Connection connection) {
		...
		try {
			...
			if (rs.next()) {
				...
				// 给当前的参数page对象设置总记录数
				page.getPage().setTotalCount(totalRecord);
			}
